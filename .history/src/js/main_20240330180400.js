...
saveBtn.addEventListener("click", function () {
  ...
  }, 7000); // Tempo em milissegundos (7 segundos)

  const date = datepicker.value;
  const time = timepicker.value;

  // Obtenha o valor selecionado no <select>
  const tipoCilio = document.getElementById("tipo_cilio").value;

  // Mapeie o valor selecionado para o texto correspondente
  let tipoCilioText = "";
  switch (tipoCilio) {
    case "brasileiro":
      tipoCilioText = "Volume Brasileiro";
      break;
    case "egipcio":
      tipoCilioText = "Volume Egípcio";
      break;
    case "hibrido":
      tipoCilioText = "Volume Híbrido";
      break;
  ...
  // Exiba o alerta com todas as informações selecionadas
  alert(
    `Seu atendimento foi marcado para o dia ${date} às ${time} horas. Tipo de cílios selecionado: ${tipoCilioText}.`
  );
});
